											...
											using (var map = GUI.Map.New(ref region, minimap, size: map_frame_size, map_scale: 1.00f, draw_markers: false))
											{
												...
												foreach (ref var row in region.IterateQuery<Minimap.GetMarkersQuery>())
												{
														...
														if (spawn.IsNotNull())
														{
															if (is_visible = spawn.IsVisibleToFaction(faction_id_tmp, faction.id)) // (faction.id == faction_id_tmp.id || spawn.flags.HasAny(Spawn.Flags.Public)) || (faction_id_tmp.id == 0 && spawn.flags.HasAny(Spawn.Flags.Neutral_Only))) //((faction.id == 0 && spawn.flags.HasAny(Spawn.Flags.Public)) || faction.id == faction_id_tmp || (faction_id_tmp == 0 && spawn.flags.HasAny(Spawn.Flags.Neutral_Only))) && marker.flags.HasAll(Minimap.Marker.Flags.Spawner))
															{
																...
																if (dormitory.IsNotNull())
																{
																	...
																	has_characters = dormitory.HasPlayableCharacters(h_faction: faction_id_tmp, h_faction_spawn: faction.id, 
																		h_player: player_asset, spawn_flags: spawn.flags);
																...
																is_selectable = !h_character_current && spawn.IsSelectableByFaction(h_faction: faction_id_tmp, 
																	h_faction_spawn: faction.id, 
																	has_characters: has_characters, 
																	is_visible: is_visible); // has_characters || faction.id == faction_id_tmp || (faction.id == 0 && spawn.flags.HasAny(Spawn.Flags.Public));
															}
													...
													if (is_visible)
													{
														...
														using (var node = map.DrawNode(sprite: marker_copy.sprite, 
														world_position: transform_copy.GetInterpolatedPosition() + new Vector2(0, -3), 
														color: (selected ? Color32BGRA.White : color).WithAlphaMult(alpha), 
														color_hovered: selected ? Color32BGRA.White : Color32BGRA.Lerp(color, Color32BGRA.White, 0.50f).WithAlphaMult(alpha)))
														{
