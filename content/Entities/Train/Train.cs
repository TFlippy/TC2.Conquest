		...
		public static class Airship
		{
			...
			[Source.Owned] ref WorldMap.Unit.Data unit, [Source.Owned] ref WorldMap.Marker.Data marker)
			{
				...
				if (route_data.IsNotNull())
				{
					...
					if (location_data.IsNotNull())
					{
						...
						{
							nav.current_route_index = (nav.current_route_index + 1) % route_data.targets.Length;
...
#if SERVER
					ref var route_target_current = ref route_data.targets.GetRefAtIndexOrNull(nav.current_route_index);
					if (route_target_current.IsNotNull())
					{
						sync |= nav.h_location_target.TryChange(route_target_current.h_location);
			...
			public partial struct WindowGUI: IGUICommand
			{
				...
				public void Draw()
				{
					...
					if (route_data.IsNotNull())
					{
						Vector2 pos_prev = transform.GetInterpolatedPosition();
						...
						for (var i = 0; i < targets.Length; i++)
						{
							...
							if (location_data.IsNotNull())
							{
								var pos = (Vector2)location_data.point;
								...
								if (i > 0) GUI.DrawLine(region.WorldToCanvas(pos_prev), region.WorldToCanvas(pos), color: GUI.col_default.WithAlpha(200), thickness: 1.00f, layer: GUI.Layer.Foreground);
								pos_prev = pos;
							}
						}
					}
